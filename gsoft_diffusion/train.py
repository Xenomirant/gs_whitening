...
def main(args):
    ...
    accelerator.register_for_checkpointing(gsoft_layers)
    for (name, param) in gsoft_layers.named_parameters():
    ...
    gsoft_layers, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
        gsoft_layers, optimizer, train_dataloader, lr_scheduler
    )
    ...
    if accelerator.is_main_process:
        ...
        accelerator.init_trackers("gsoft", config=tracker_config)
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                if accelerator.sync_gradients:
                    accelerator.clip_grad_norm_(gsoft_layers.parameters(), args.max_grad_norm)
                optimizer.step()
                lr_scheduler.step()
                optimizer.zero_grad()
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        unwrap_model(unet).save_attn_procs(os.path.join(save_path))
